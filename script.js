(function (global, factory) {
  ...
  class index {
    ...
    next_block(x, y, left, top) {
      ...
      function block_set_5(x, y) {
        if (extend(x, y))
          return { v: true, h: false, in: true, col: top.col, id: top.id };
        return block_set_4(x, y);
      }
      ...
      function extend(x, y) {
        if (!active_position(x, y, 1 - context.roundness) && !context.simple)
          return false;
      ...
      function active_position(x, y, fuzzy) {
        ...
        let xa =
          Math.pow(x - context.xdim / 2, 2) /
          Math.pow(context.radius_x * fuzziness, 2);
        let ya =
          Math.pow(y - context.ydim / 2, 2) /
          Math.pow(context.radius_y * fuzziness, 2);
        return xa + ya < 1;
      }
    }
  }
    ...
    for (let i = 0; i < grid.length; i++) {
      for (let j = 0; j < grid[i].length; j++) {
        ...
        if (cell.h && cell.v && cell.in)
          nw_corners.push({ x1: j, y1: i, col: cell.col, id: cell.id });
      }
    }
    return nw_corners;
  }
  ...
  const palettes = [
    ...
    {
      name: "Doodles light",
      colors: ["#f6e1f4", "#e6e1f4", "#d6e1f4", "#c6e1f4", "#b6e1f4"]
    },
    {
      name: "Fidenza",
      colors: [
    ...
    {
      name: "Doodles",
      colors: ["#f9b8b8", "#f3f5c8", "#abf8f2", "#cad4f3", "#fad8fb"]
    },
    {
      name: "Doodles Rainbow",
      colors: ["#fea3aa", "#f8b88b", "#faf884", "#b2cefe", "#f2a2e8"]
    },
    {
      name: "roygbiv-toned",
      colors: [
  ...
  const sketch = (p) => {
    ...
    function get_neighborhood(ps, rs, dir) {
      ...
      let ms = union(
        ns,
        flatten(
          ns.map((n) => get_neighbors(n, rs, dir)),
          equal_rect
        ),
        equal_rect
      );
